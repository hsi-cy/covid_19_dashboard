...
globalConfirmedByCountries = globalConfirmed.groupby(
    'Country/Region').sum().transpose()
globalDeathsByCountries = globalDeaths.groupby(
    'Country/Region').sum().transpose()
globalRecoveredByCountries = globalRecovered.groupby(
    'Country/Region').sum().transpose()

globalConfirmedByCountriesDateIncluded = globalConfirmedByCountries.reset_index(
).rename(columns={'index': 'Date'})
globalDeathsByCountriesDateIncluded = globalDeathsByCountries.reset_index(
).rename(columns={'index': 'Date'})
globalRecoveredByCountriesDateIncluded = globalRecoveredByCountries.reset_index(
).rename(columns={'index': 'Date'})
